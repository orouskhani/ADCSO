...
int main(int argc, char* argv[])
{
	...
	int i_Iteration[i_FuncNum]={1, 1, 2000}; // Defines the total iteration number in one cycle.
  ...
  double d_InitL[i_FuncNum]={-32.678, 2.56, -32.678};       // Defines the left border of the initial ranges.
  double d_InitR[i_FuncNum]={32.678, 5.12, +32.678};       // Defines the right border of the initial ranges.
...
//maysam
              for(int k = 0 ; k < i_Dim[(i-1)] ; k++){
				w[k] = 0.15 + (((double)( i_Dim[i-1] - k )) / ((double)( 2 * i_Dim[i-1] ))) ;
				//w[k] = ( ( wStart-wEnd ) / (1 + exp(-1*u*(k - partitionSet * i_Dim[i-1])) ) )+ wEnd;
				wWriter<<w[k]<<" "<< ( 2 * i_Dim[i-1] )<<endl;
				
		
		}

		for(int k = 0 ; k < i_Dim[(i-1)] ; k++){
			IC[k] = 2 - (((double)( i_Dim[i-1] - k )) / ((double)( 2 * i_Dim[i-1] ))) ;
			cWriter<<IC[k]<<endl;
		}
