...
void CSO_Tracing_Mode(tcat t_cat[],int i_tg,int i_PSize,int i_Dim,double d_MaxVel , double w[], double IC[])
{
  int i;
  ...
  for(i=0;i<i_Dim;i++)
  {
     ...
     //t_cat[i_tg].d_vel[i] = w[i]*t_cat[i_tg].d_vel[i] + (Rand()*IC[i]*(t_cat[i_PSize].d_pos[i]-t_cat[i_tg].d_pos[i])); // Update the velocity of the i_tg cat.
	 t_cat[i_tg].d_vel[i] = w[i]*t_cat[i_tg].d_vel[i] + (Rand()*IC[i]*(t_cat[i_PSize].d_pos[i]-t_cat[i_tg].d_pos[i])); // Update the velocity of the i_tg cat.
    ...
    //t_cat[i_tg].d_pos[i+1]=(t_cat[i_tg].d_pos[i+1]+t_cat[i_tg].d_pos[i])/2+(t_cat[i_tg].d_vel[i+1]+t_cat[i_tg].d_vel[i])/2; // Update the coordinate of the i_tg cat.
     //double* landa = new double[9*i/10+1];
	 //for(int k=0; k<9*i/10; k++)
	     //landa[k]=0.5/pow(2,k);
	 //landa[9*i/10]=landa[9*i/10-1];
     //for(int k=i; k>=9*i/10; k--)
 	 //{
	     //t_cat[i_tg].d_pos[i]+= landa[i-k]*t_cat[i_tg].d_pos[k];
		 //t_cat[i_tg].d_vel[i]+= landa[i-k]*t_cat[i_tg].d_vel[k];
	 //}
    //t_cat[i_tg].d_pos[i]+= t_cat[i_tg].d_vel[i];
	t_cat[i_tg].d_pos[i]=(t_cat[i_tg].d_pos[i]+((0.6*t_cat[i_tg].d_pos[i+1]+0.4*t_cat[i_tg].d_pos[i+2])/2)+((0.6*t_cat[i_tg].d_pos[i-1]+0.4*t_cat[i_tg].d_pos[i-2])/2) + t_cat[i_tg].d_vel[i]+((0.6*t_cat[i_tg].d_vel[i+1]+0.4*t_cat[i_tg].d_vel[i+2])/2)+((0.6*t_cat[i_tg].d_vel[i-1]+0.4*t_cat[i_tg].d_vel[i-2])/2))/2;   
...
void CSO_Movement(tcat t_cat[],int i_PSize,int i_Dim,bool b_Maximize,int i_FN,double d_MaxVel,double w[],double IC[])
{
  int i_NTracing;                       // Contains the number of cats to be selected to move in the tracing mode.
  double d_tmp;
  int i, *i_seq;
  ...
  for(i=0;i<i_PSize;i++)                // All the cats should take turns to move according to their STFlags.
  {
    if(t_cat[i].STFlag)                 // This cat should go into the Tracing mode.
      CSO_Tracing_Mode(t_cat,i,i_PSize,i_Dim,d_MaxVel,w,IC);
